...
class _AddMachineFormState extends State<AddMachineForm> {
  ...
  initList() async {
    ...
    setState(() {
      defaultChaine = chaines.isEmpty ? 0 : chaines.first["id"];
      defaultEtat = etats.isEmpty ? 0 : etats.first["id"];
      defaultRef = refs.isEmpty ? 0 : refs.first["id"];
      defaultParc = 0;
      isLoading = false;
    });
  }
    ...
    return Form(
      ...
      child: Column(
        children: isLoading
            ...
            : chaines.isEmpty || refs.isEmpty || etats.isEmpty
                ? [
                    const Text(
                      "Veuillez vérifier que vous avez toutes les listes complètes : Références , états machines et chaînes.",
